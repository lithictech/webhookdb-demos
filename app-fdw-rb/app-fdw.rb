...
app_customer = DB[table].
  returning.
  insert(name: name, note: 'Great customer!', stripe_id: stripe_customer.id).
  first
...
found = loop do
  found = DB[table].
    join(Sequel[whdb_schema][whdb_stripe_customers], stripe_id: :stripe_id).
    where(Sequel[table][:stripe_id] => app_customer[:stripe_id]).
    select(:id, :note, Sequel[table][:name], Sequel[table][:stripe_id], Sequel[:pk].as(:whdb_pk), :data).
    first
...
puts "Found customer, JOINed with WebhookDB's FDW: #{found}"
